...
ActionCareerWEThornsisterTargetWall._check_segment = function (self, prev_position, position, forward)
	if position then
		...
		local result, hit_position, length = PhysicsWorld.immediate_raycast(physics_world, from, normalized_direction, distance, "closest", "collision_filter", "filter_player_mover")
		...
		if result then
			...
			if prev_position then
				...
				hit_actors, actor_count = PhysicsWorld.immediate_overlap(physics_world, "position", overlap_pos, "rotation", overlap_rot, "size", overlap_size, "shape", "oobb", "collision_filter", "filter_player_mover")
			end
