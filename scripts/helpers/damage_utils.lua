...
DamageUtils.get_unit_armor = function (target_unit, hit_zone_name)
	...
	elseif target_unit then
		local unit_data_armor = Unit.get_data(target_unit, "armor") or 1
...
DamageUtils.get_breed_damage_multiplier_type = function (target_unit, hit_zone_name)
	...
	if target_unit and not breed and Unit.get_data(target_unit, "is_dummy") and hit_zone_name == "head" then
...
DamageUtils.get_boost_curve_multiplier = function (curve, percent)
	...
	local a = (-p0/2 + (p1*3)/2) - (p2*3)/2 + p3/2
	local b = (p0 - (p1*5)/2 + p2*2) - p3/2
	local c = -p0/2 + p2/2
	local d = p1
...
DamageUtils.calculate_damage = function (damage_output, target_unit, attacker_unit, hit_zone_name, original_power_level, boost_curve, boost_curve_multiplier, is_critical_strike, damage_profile, target_index, backstab_multiplier)
	...
	local is_enemy_damage = attacker_unit and not table.contains(PLAYER_AND_BOT_UNITS, attacker_unit)
	local is_player_on_player_damage = attacker_unit and table.contains(PLAYER_AND_BOT_UNITS, target_unit) and table.contains(PLAYER_AND_BOT_UNITS, attacker_unit)
	local target_unit_armor, target_unit_primary_armor = nil
	...
	local damage_boost_damage = 0

	if has_damage_boost then
		local damage_boost_armor = nil

		if target_unit_armor == 2 or target_unit_armor == 5 or target_unit_armor == 6 then
			damage_boost_armor = 1
		else
			damage_boost_armor = target_unit_armor
		end

		local damage_boost_target_damages = damage_output[damage_boost_armor] or ((damage_boost_armor ~= 0 or 0) and damage_output[1])
	...
	if not is_enemy_damage then
		...
		if multiplier_type == "headshot" or multiplier_type == "weakspot" or multiplier_type == "protected_weakspot" then
			if 0 < damage then
				if target_unit_armor == 3 then
					head_shot_boost_amount = head_shot_boost_amount + ((target_settings and (target_settings.headshot_boost_boss or target_settings.headshot_boost)) or 0.5)
				else
		...
		if is_critical_strike then
			local crit_boost = damage_profile.crit_boost or 0.5
		...
		if boost_curve and (0 < boost_amount or 0 < head_shot_boost_amount) then
			...
			if head_shot_boost_amount then
				...
				if not units[hit_unit] and (not ignore_attacker_unit or hit_unit ~= attacker_unit) and not ignore_damage then
					...
					if hit_zone_name ~= "afro" then
						...
						if max_damage_radius < hit_distance then
							distance_factor = (hit_distance - max_damage_radius)/(radius - max_damage_radius) - 1
						end
...
DamageUtils.add_damage_network_player = function (damage_profile, target_index, power_level, hit_unit, attacker_unit, hit_zone_name, attack_direction, damage_source, hit_ragdoll_actor, boost_curve_multiplier, is_critical_strike, backstab_multiplier, added_dot)
	...
	if player_manager.is_server and script_data.debug_show_damage_numbers then
		local is_alive = AiUtils.unit_alive(hit_unit)
		local debug_text_manager = Managers.state.debug_text

		if is_alive then
			local color_modifier_red = math.min(damage_amount*4 + 120, 255)
			local color_modifier_green = math.max(damage_amount*4 - 200, 0)
			local color1 = Vector3(color_modifier_red, color_modifier_green, 0)
			local debug_text_manager = Managers.state.debug_text
			local random_x = math.random(-30, 30)*0.01
			local text_size = damage_amount*0.005 + 0.18
			local duration = 2.2
			...
			debug_text_manager.output_unit_text(debug_text_manager, "" .. damage_amount, text_size, hit_unit, 0, Vector3(random_x, 0, 2), duration, "damage", color1, "player_1", true, true)
		end
	end
...
DamageUtils.apply_buffs_to_damage = function (current_damage, attacked_unit, attacker_unit, damage_source, victim_units, damage_type, buff_attack_type)
	...
	if ScriptUnit.has_extension(attacked_unit, "buff_system") then
		...
		if damage_type ~= "overcharge" and 0 < damage and not is_knocked_down then
		...
		if is_invulnerable or (has_gromril_armour and damage_source ~= "ground_impact") then
			damage = 0
		end

		if has_gromril_armour and damage_source ~= "ground_impact" then
...
DamageUtils.debug_deal_damage = function (victim_unit, attack_template_name, hit_zone_name)
	...
	weapon_system.send_rpc_attack_hit(weapon_system, damage_source_id, attacker_unit_id, hit_unit_id, hit_zone_id, attack_direction, damage_profile_id, "power_level", 100, "hit_target_index", nil, "blocking", false, "shield_break_procced", false, "boost_curve_multiplier", 1, "is_critical_strike", false)
...
DamageUtils.is_enemy = function (unit)
	...
	return breed and not breed.allied
end
...
DamageUtils.process_projectile_hit = function (world, damage_source, owner_unit, is_server, raycast_result, current_action, direction, check_buffs, target, ignore_list, is_critical_strike, power_level, override_damage_profile)
	...
	for i = 1, num_hits, 1 do
		...
		if not attack_hit_self and hit_valid_target then
			...
			if breed and breed.allied then
				return hit_data
			end
			...
			if not is_character then
				...
				local is_dummy_unit = not is_level_unit and Unit.get_data(hit_unit, "is_dummy")
				...
				if is_dummy_unit and not hit_units[hit_unit] then
					...
					return hit_data
				elseif is_level_unit and not hit_units[hit_unit] and (GameSettingsDevelopment.allow_ranged_attacks_to_damage_props or Unit.get_data(hit_unit, "allow_ranged_damage") or allow_ranged_damage) and has_health_extension then
					hit_units[hit_unit] = true
					amount_of_mass_hit = amount_of_mass_hit + 1
					local target_index = math.ceil(amount_of_mass_hit)
				...
				elseif not is_level_unit and allow_ranged_damage and has_health_extension and not owner then
					...
					if not is_inventory_item then
						...
						if hit_unit_owner == nil or hit_unit_owner.player_unit == nil then
							...
							if allow_ranged_damage and hit_unit and Unit.alive(hit_unit) and hit_actor then
								local unit_set_flow_variable = Unit.set_flow_variable
			...
			elseif not hit_units[hit_unit] and is_target and ((is_player and (not owner_is_player or DamageUtils.allow_friendly_fire_ranged(difficulty_settings, owner_is_player))) or breed) then
				...
				elseif hit_unit_player and hit_actor == Unit.actor(hit_unit, "c_afro") then
					...
					if Unit.alive(owner_unit) and hit_unit_player then
...
DamageUtils.redirect_shield_hit = function (hit_unit, hit_actor)
	local potential_hit_unit_owner = Unit.get_data(hit_unit, "shield_owner_unit")
	...
	if potential_hit_unit_owner then
		...
		local new_hit_actor = Unit.actor(potential_hit_unit_owner, Unit.find_actor(potential_hit_unit_owner, "c_leftforearm"))
