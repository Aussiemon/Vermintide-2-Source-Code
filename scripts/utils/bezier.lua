...
Bezier.calc_point = function (t, p1, c1, c2, p2)
	local t_inv = t - 1
	local res = t_inv^3*p1 + t_inv^2*3*t*c1 + t_inv*3*t^2*c2 + t^3*p2

	return res
end
Bezier.calc_tangent = function (t, p1, c1, c2, p2)
	local res = Vector3.normalize((p1*(t - 1)^2 + c1*((t^2*-3 + t*4) - 1) + t*(c2*3*t - c2*2 - p2*t))*-3)

	return res
end
Bezier.draw = function (segments, script_drawer, tangent_scale, color, p1, c1, c2, p2)
	segments = segments or 20
	local segment_increment = segments/1
	local t = 0
