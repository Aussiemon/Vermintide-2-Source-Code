...
ProfileRequester.request_profile = function (self, peer_id, local_player_id, profile_name, career_name)
	...
	if self._is_server then
		self:_request_profile(peer_id, local_player_id, self._request_id, profile_index, career_index)
	else
		self._network_transmit:send_rpc_server("rpc_request_profile", local_player_id, self._request_id, profile_index, career_index)
	end
end

ProfileRequester._request_profile = function (self, peer_id, local_player_id, request_id, profile_index, career_index)
	local profile_name, career_name = hero_and_career_name_from_index(profile_index, career_index)
	local allowed_to_switch_to_profile = Managers.mechanism:profile_available_for_peer(peer_id, local_player_id, profile_name, career_name)
	local result_id = nil
	...
	if self._peer_id == peer_id then
		self:rpc_request_profile_reply(peer_id, local_player_id, request_id, profile_index, career_index, result_id)
	else
		self._network_transmit:send_rpc("rpc_request_profile_reply", peer_id, local_player_id, request_id, profile_index, career_index, result_id)
	end
end
...
ProfileRequester.rpc_request_profile = function (self, sender, local_player_id, request_id, profile_index, career_index)
	self:_request_profile(sender, local_player_id, request_id, profile_index, career_index)
end

ProfileRequester.rpc_request_profile_reply = function (self, sender, local_player_id, request_id, profile_index, career_index, result_id)
