...
GameModeVersus.server_update = function (self, t, dt)
	...
	elseif state == "pre_start_round_state" then
		...
		local time_left = math.floor(self.pre_round_start_timer - t)
...
GameModeVersus.player_entered_game_session = function (self, peer_id, local_player_id, wanted_party_index)
	...
	local profile_index, career_index = self._mechanism:update_wanted_hero_character(peer_id, local_player_id, wanted_party_id)
...
GameModeVersus._assign_peer_to_wanted_hero_profile = function (self, peer_id, local_player_id, party_id)
	...
	local saved_profile_idx, saved_career_idx = self._mechanism:get_saved_hero(peer_id, local_player_id)
...
GameModeVersus.profile_changed = function (self, peer_id, local_player_id, profile_index, career_index, is_bot)
	...
	if party.party_id == 0 then
		return
	end

	self._mechanism:set_saved_hero(peer_id, local_player_id, party.party_id, profile_index, career_index)
end
