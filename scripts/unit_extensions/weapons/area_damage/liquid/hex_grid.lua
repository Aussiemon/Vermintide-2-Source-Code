...
HexGrid.find_position = function (self, i, j, k)
	...
	local x = ((j - 1)*0.5 + i)*self._x_cell_size
...
HexGrid.sample_grid = function (self, samples, z, multiplier)
	...
	local root = self._root_position:unbox()
	local min_x = root.x + xy_extents*self._x_cell_size*(multiplier - 1)
	local max_x = root.x + (xy_extents*(multiplier*1 + 1) + 1)*self._x_cell_size
	local min_y = root.y + xy_extents*self._x_cell_size*(multiplier - 1)
	local max_y = root.y + (xy_extents*(multiplier*1 + 1) + 1)*self._y_cell_size
	...
	for index = 1, samples, 1 do
		...
		if i < min_i or max_i < i or j < min_j or max_j < j then
			...
			local i_val = i - min_i
			local j_val = j - min_j
			local r = (i_val%2 == 0 and (i_val*125)/i_span) or (i_val*125)/i_span + 125
			local g = (j_val%2 == 0 and (j_val*125)/j_span) or (j_val*125)/j_span + 125
			local b = 0
			color = Color(r, g, b)
		end
