		...
		if hit_unit ~= player_unit and Unit.alive(hit_unit) and not hit_units[hit_unit] then
			...
			if unit_is_character and is_valid_player_status then
				...
				if attack_cone < dot then
					local los = not PhysicsWorld.immediate_raycast(physics_world, player_position, to_target_normalized, attack_half_range * 2, "closest", "collision_filter", "filter_husk_in_line_of_sight")

					if is_player_unit then
						los = los or PerceptionUtils.is_position_in_line_of_sight(player_unit, player_position, hit_unit_pos + horizontal_offset, physics_world, "filter_husk_in_line_of_sight")
						los = los or PerceptionUtils.is_position_in_line_of_sight(player_unit, player_position, hit_unit_pos - horizontal_offset, physics_world, "filter_husk_in_line_of_sight")
						los = los or PerceptionUtils.is_position_in_line_of_sight(player_unit, player_position, hit_unit_pos + vertical_offset, physics_world, "filter_husk_in_line_of_sight")
						los = los or PerceptionUtils.is_position_in_line_of_sight(player_unit, player_position, hit_unit_pos - vertical_offset, physics_world, "filter_husk_in_line_of_sight")
					end
