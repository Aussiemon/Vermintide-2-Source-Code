...
PlayerUnitGhostModeExtension.update = function (self, unit, input, dt, context, t)
	if self._ghost_mode_active then
		self._range = self:get_distance_from_players(self._unit)

		if self._range and self._range < self._latest_range_update - 1 or self._range > self._latest_range_update + 1 then
