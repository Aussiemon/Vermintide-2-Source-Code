...
OverchargeBarUI.set_charge_bar_fraction = function (self, overcharge_fraction, min_threshold_fraction, max_threshold_fraction, anim_blend_overcharge)
	...
	bar_color[3] = color[3]
	bar_color[4] = color[4]
	style.min_threshold.offset[1] = min_threshold_fraction*bar_size[1] + 3
	style.max_threshold.offset[1] = max_threshold_fraction*bar_size[1] + 3
	local pulse_speed = 10
	local pulse_global_fraction = math.min(math.max(overcharge_fraction - max_threshold_fraction, 0)/(max_threshold_fraction - 1)*1.3, 1)
	local pulse_fraction = math.sin(Application.time_since_launch()*pulse_speed)*0.5 + 0.5
	local pulse_alpha = (pulse_fraction*155 + 100)*pulse_global_fraction
