...
StoreWindowCategoryList._populate_list = function (self, layout)
	...
	for i, entry in ipairs(layout) do
		...
		local items = self:_get_items_by_path(path)
		local items_owned = 0
...
StoreWindowCategoryList._animate_list_entry = function (self, content, style, dt, list_hovered)
	...
	local counter_text_style = style.counter
	local counter_text_color = counter_text_style.text_color
	local counter_default_text_color = counter_text_style.default_text_color
	local counter_select_text_color = counter_text_style.select_text_color

	Colors.lerp_color_tables(counter_default_text_color, counter_select_text_color, combined_progress, counter_text_color)
...
StoreWindowCategoryList._on_list_index_selected = function (self, index)
	local layout = self._layout
	local entry = layout[index]
