...
StoreWindowItemPreview._sync_presentation_item = function (self, force_update)
	...
	if selected_product ~= self._selected_product or force_update then
		...
		if reset_presentation then
			...
			self:_update_unlock_button_width(unlock_button_width_offset)
		end
...
StoreWindowItemPreview._update_unlock_button_width = function (self, width_offset)
	local top_widgets_by_name = self._top_widgets_by_name
	...
	local currency_icon_width = (present_currency and currency_icon_style.texture_size[1]) or 0
	...
	local currency_text = (present_currency and "9999") or ""
