...
StoreWindowCategoryItemList.on_exit = function (self, params)
...
StoreWindowCategoryItemList._animate_list_entries = function (self, dt)
	...
	for _, widget in ipairs(list_widgets) do
		...
		parent:animate_item_product(widget, dt, list_hovered)
	end
end
...
StoreWindowCategoryItemList._update_item_list = function (self)
	...
	if items then
		...
		for _, item in ipairs(items) do
			...
			if backend_items:has_item(item_key) then
	...
	local counter_text = "(" .. tostring(items_owned) .. "/" .. tostring(num_items) .. ")"
	...
	self:_set_title_texts(Localize(current_page_display_name), counter_text)
...
StoreWindowCategoryItemList._set_title_texts = function (self, title_text, counter_text)
	local widgets_by_name = self._widgets_by_name
	local title_widget = widgets_by_name.title_text
	local counter_widget = widgets_by_name.counter_text
	title_widget.content.text = title_text
	counter_widget.content.text = counter_text
end
...
StoreWindowCategoryItemList._on_list_index_selected = function (self, index)
	local layout = self._layout
...
StoreWindowCategoryItemList._destroy_product_widgets = function (self)
	local parent = self._parent
	local layout = self._layout
...
StoreWindowCategoryItemList._handle_gamepad_grid_selection = function (self, input_service)
	...
	if new_index then
		...
		self:_scroll_to_list_index(new_index)
	end
end
