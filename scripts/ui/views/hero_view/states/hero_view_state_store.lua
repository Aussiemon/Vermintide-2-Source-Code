...
HeroViewStateStore._trigget_welcome_popup = function (self)
	...
	for name, settings in pairs(DLCSettings) do
		if settings.unlock_settings then
			local dlc_name, settings = next(settings.unlock_settings)

			if settings and settings.class == "UnlockDlc" and Managers.unlock:is_dlc_unlocked(dlc_name) then
				reward_data[#reward_data + 1] = {
					value = base_value,
					dlc = dlc_name
				}
				includes_dlc_rewards = true
			end
		end
	end

	local welcome_text_2 = (includes_dlc_rewards and "welcome_currency_popup_intro_description_2") or "welcome_currency_popup_intro_description_2_no_dlc"
	local layout = {
		{
			type = "body",
			settings = {
	...
	for i = 1, #reward_data, 1 do
		...
		local is_dlc_reward = data.dlc ~= nil
		local initial_bonus = data.initial_bonus
		local value = data.value

		if is_dlc_reward then
			layout[#layout + 1] = {
				type = "summary_entry",
				settings = {
					localize = true,
		...
		elseif initial_bonus then
			local welcome_gift_text = "welcome_currency_popup_intro_welcome_gift"
			layout[#layout + 1] = {
				type = "summary_entry",
				settings = {
					localize = true,
					text = welcome_gift_text,
					value = value
				}
			}
		end
	...
	self:block_input()
	Managers.backend:set_user_data("store_initial_shilling_summary", total_amount)
end
