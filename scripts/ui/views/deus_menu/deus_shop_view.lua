...
local definitions = local_require("scripts/ui/views/deus_menu/deus_shop_view_definitions")
...
DeusShopView._update_during_finishing = function (self, dt, t)
	local widgets_by_name = self._widgets_by_name
	local bottom_text_widget_content = widgets_by_name.bottom_text.content
	local timer_text = Localize("deus_shrine_continue_in")
	local timer = math.round(self._final_countdown)
	bottom_text_widget_content.text = string.format(timer_text, timer)
	widgets_by_name.ready_button.content.button_hotspot.disable_button = true
...
DeusShopView._hide_all_player_units = function (self)
	local widgets_by_name = self._widgets_by_name

	for i = 1, 4, 1 do
		local portrait_widget_name = "player_portrait_" .. i
		local text_widget_name = "player_texts_" .. i
		local player_portrait = widgets_by_name[portrait_widget_name]
		local player_texts = widgets_by_name[text_widget_name]
		player_portrait.content.visible = false
		player_texts.content.visible = false

		UIRenderer.set_element_visible(self.ui_renderer, player_portrait.element, false)
	end
end

DeusShopView._update_player_portraits = function (self, player_data)
	self:_hide_all_player_units()
	...
	for i, data in ipairs(player_data) do
		...
		if data.peer_state == peer_states.DONE_BUYING then
			player_texts.content.state_text = Localize("deus_peer_state_ready")
		elseif data.peer_state == peer_states.READY_TO_BUY then
			player_texts.content.state_text = Localize("deus_peer_state_not_ready")
		else
			player_texts.content.state_text = Localize("deus_peer_state_joining")
		end
