...
ImguiDeusLoadLevel.draw = function (self, is_open)
	...
	Imgui.begin_window("DeusLoadLevel", "always_auto_resize")
	...
	if mechanism_name ~= "deus" then
		...
		local full_name = level_name .. "_" .. deus_level.themes[self._theme_index] .. "_path" .. deus_level.paths[self._path_index]

		if Imgui.button("Load") then
			local mechanism = Managers.mechanism:game_mechanism()

			mechanism:debug_load_deus_level(full_name, difficulties[self._difficulty_index], self._progress, self._level_seed)
		end
	end
