...
ImguiBuffsDebug._display_stat_buffs = function (self, stat_buffs)
	if Imgui.TreeNode("Stat Buffs") then
		...
		if stat_buffs then
			Imgui.Separator()
			Imgui.Text(string.format("%-30s%8s%12s%13s%14s", "Name", "Bonus", "Multiplier", "Proc Chance", "Final Value"))
			Imgui.Separator()
			...
			for name, data in pairs(stat_buffs) do
				if not table.is_empty(data) then
					...
					for i = 1, #data, 1 do
						...
						final_value = final_value * (1 + multiplier) + bonus

						Imgui.Text(string.format("%-30s%8.2f%12.2f%13.2f%14.2f", (i == 1 and name) or "", bonus, multiplier, proc_chance, final_value))
					end
...
ImguiBuffsDebug._display_event_buffs = function (self, event_buffs)
	if Imgui.TreeNode("Event Buffs") then
		if event_buffs then
			Imgui.Separator()
			Imgui.Text(string.format("%-53s%8s%12s%13s", "Name", "Bonus", "Multiplier", "Proc Chance"))
			Imgui.Separator()
				...
				if not table.is_empty(data) then
					if Imgui.TreeNode(name) then
						for index, buff in pairs(data) do
							...
							Imgui.Text(string.format("%-50s%8.2f%12.2f%13.2f", buff_name, bonus, multiplier, proc_chance))
						end
